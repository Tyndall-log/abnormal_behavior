...
# 비디오 파일이 있는 폴더 경로
video_folder_path = os.path.abspath("G:/abnormal_behavior_wsl/Dataset/assult/outsidedoor_06")
    ...
    for video_file in video_files:
        ...
        if os.path.exists(output_file):
            with open(output_file, "rb") as f:
                existing_data = bson.decode_all(f.read())
                if existing_data:
                    last_frame = existing_data[-1]['frame_id']
                    # 영상이 이미 완료되었는지 확인
        ...
        with tqdm(total=total_frames, initial=frame_index, desc=f"Processing {os.path.basename(video_file)}") as video_pbar:
            ...
            while cap.isOpened():
                ...
                if success:
                    ...
                    keypoints = results[0].keypoints.xy.cpu().numpy().tolist() if results[0].keypoints is not None else []
