...
video_folder_paths = [
	"G:/내 드라이브/machine_learning/dataset/assult/outsidedoor_06/",
]
...
with tqdm(total=len(video_files), desc="training") as overall_pbar:
	for video_file in video_files:
		...
		for frame_idx in range(event_start_frame, event_end_frame):
			...
			for obj_id, obj_data in data.items():
				data_merged = obj_data["angular_velocities"] + obj_data["linear_velocities"]
	...
	save_filename = "train_data(extra).csv"
